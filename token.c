...
int get_token(TokenData *data) {
  ...
  if (isalpha(last_char)) {
    if (data->identifier_str.items != NULL) {
      free(data->identifier_str.items);
      data->identifier_str.items = NULL;
      data->identifier_str.count = 0;
      data->identifier_str.capacity = 0;
